...
client.on("messageCreate", async (message) => {
  ...
  if (command === "spotify") {
    if (!message.member.voice.channel) return message.reply("❌ Join a voice channel first!");
    const query = args.join(" ");
    if (!query) return message.reply("❌ Provide a song name or playlist link!");
    ...
    try {
      ...
      console.error(err);
      message.reply("❌ Error playing Spotify song.");
    }
  }
...
client.login(MTQ1Nzc5Nzk4Mzg2ODQyMDMzMg.GjbUzz.KHjhox2Vyuf3wI6kaambj5NN5EPTYqNeQU2fIc);
